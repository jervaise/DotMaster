...
-- Create a tab system
function DotMaster_Components.CreateTabSystem(parent, width, height)
  if DM.Debug then
    DM.Debug:Loading("Creating tab system")
  end
  ...
  local container = CreateFrame("Frame", nil, parent)
  container:SetSize(width or 400, height or 400)

  -- Tab header
  local tabHeader = CreateFrame("Frame", nil, container)
  tabHeader:SetPoint("TOPLEFT", container, "TOPLEFT", 0, 0)
  tabHeader:SetPoint("TOPRIGHT", container, "TOPRIGHT", 0, 0)
  tabHeader:SetHeight(30)

  -- Content area
  local contentArea = CreateFrame("Frame", nil, container)
  contentArea:SetPoint("TOPLEFT", tabHeader, "BOTTOMLEFT", 0, -5)
  contentArea:SetPoint("BOTTOMRIGHT", container, "BOTTOMRIGHT", 0, 0)

  -- Function to add a tab
  function tabSystem:AddTab(name, contentFunc)
    local tabIndex = #tabs + 1

    -- Create tab button
    local tabButton = CreateFrame("Button", nil, tabHeader)
    tabButton:SetHeight(30)

    -- Calculate button width based on number of tabs
    local buttonWidth = math.min(100, width / (tabIndex + 1))
    tabButton:SetWidth(buttonWidth)

    -- Position the button
    if tabIndex == 1 then
      tabButton:SetPoint("TOPLEFT", tabHeader, "TOPLEFT", 0, 0)
    else
      tabButton:SetPoint("TOPLEFT", tabButtons[tabIndex - 1], "TOPRIGHT", 2, 0)
    end
  ...
  function tabSystem:SelectTab(index)
    ...
    for i, tab in ipairs(tabs) do
      tab.content:Hide()
      tab.button.bg:SetColorTexture(0.1, 0.1, 0.1, 0.8)
    end

    -- Initialize tab content if needed
    if not tabs[index].initialized and tabs[index].contentFunc then
      tabs[index].contentFunc(tabs[index].content)
      tabs[index].initialized = true
    end

    -- Show the selected tab
    tabs[index].content:Show()
    tabs[index].button.bg:SetColorTexture(0.3, 0.3, 0.3, 0.8)
    activeTab = index
  end
...
function DM:CreateGUI()
  ...
  print("|cFFFF00FFDEBUG:|r DM:CreateGUI in gui_common.lua called directly")

  if DM.Debug then
    DM.Debug:Loading("DM:CreateGUI in gui_common.lua called")
  end
...
-- Expose the CreateGUI function to the DM.GUI namespace
DM.GUI.CreateGUI = DM.CreateGUI
